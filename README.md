...
<br>
 ...
 💭 짧은 회고
  ...
  - 우지훈 : 처음 해보는 크롤링이었지만, 혼자서 주소를 타고 들어가 데이터를 긁어오는 모습이 정말 신기했다. MySQL에 맞는 DB와 테이블을 설계해 데이터를 저장하고, 상관관계를 분석하여 리콜 데이터를 전처리했다. 특히 리콜 데이터의 모델명을 참조한 브랜드 정제 작업이 가장 까다로웠다. 더 나아가, 크롤링 기능을 활용한 점메추 봇 MK2을 만들 계획이다.
